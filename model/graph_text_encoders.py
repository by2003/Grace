...
def encode_code_graph(graph: nx.Graph, node_encoder: str = None, edge_encoder: str = None, max_code_len: int = 40) -> str:
    """Encode a code graph using rich node attributes.
 
     Each node in the graph is expected to carry at least the following
     attributes (additional attributes are ignored gracefully):
         - ``type``:  lexical category such as *operator*, *identifier* …
         - ``code``:  raw source substring represented by the node.
         - ``parso_type``:  fully–qualified class name from *parso* AST (optional).
 
     The function produces a human-readable textual description so that an
     LLM can reason over the structure as well as the semantics of the code
     elements.
 
     Args:
         graph: ``networkx`` graph whose nodes contain the above attributes.
         node_encoder: *unused* kept for API compatibility.
         edge_encoder: *unused* kept for API compatibility.
         max_code_len: truncate long code snippets to this many characters.
 
     Returns:
         A multi-line string describing nodes first and then edges.
     """


    out_lines = []
    out_lines.append(
        f"G is a program graph with {graph.number_of_nodes()} nodes and {graph.number_of_edges()} edges."
    )
 
    # --- Node descriptions --------------------------------------------------------
    out_lines.append("Node descriptions:")
    for node_id, data in graph.nodes(data=True):
        node_type = data.get("type", "unknown")
        code_snippet = str(data.get("code", "")).replace("\n", "\\n")
        if len(code_snippet) > max_code_len:
            code_snippet = code_snippet[:max_code_len] + "…"
        parso_type = data.get("parso_type")
        # Clean parso_type to only keep class name if present
        if parso_type:
            parso_type = str(parso_type).split(".")[-1].rstrip("'>")
        else:
            parso_type = "N/A"
 
        out_lines.append(
            f" Node {node_id}: type={node_type}, parso={parso_type}, code=\"{code_snippet}\""
        )
    ...
    if graph.number_of_edges() > 0:
        ...
        for src, dst, edata in graph.edges(data=True):
            e_type = edata.get("type", "linked")
            out_lines.append(f"  ({src}) {arrow}[{e_type}]{arrow} ({dst})")
